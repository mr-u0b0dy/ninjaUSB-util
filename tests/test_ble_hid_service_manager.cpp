...
const char* report_type_to_string(HIDReportType report_type) {
    switch (report_type) {
        case HIDReportType::KEYBOARD_INPUT: return "Keyboard Input";
        case HIDReportType::CONSUMER_CONTROL: return "Consumer Control";
        case HIDReportType::MOUSE_INPUT: return "Mouse Input";
        case HIDReportType::KEYBOARD_OUTPUT: return "Keyboard Output";
        case HIDReportType::FEATURE: return "Feature";
...
public:
    ...
    bool send_keyboard_report(const std::array<std::uint8_t, 8>& report) {
        ...
        if (it == characteristic_map_.end()) return false;
...
void test_keyboard_report_sending() {
    ...
    std::array<std::uint8_t, 8> keyboard_report = {0x01, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00}; // Ctrl+A
...
void test_utility_functions() {
    ...
    assert(std::string(ble_hid::report_type_to_string(ble_hid::HIDReportType::KEYBOARD_INPUT)) == "Keyboard Input");
    assert(std::string(ble_hid::report_type_to_string(ble_hid::HIDReportType::CONSUMER_CONTROL)) == "Consumer Control");
...
int main() {
    ...
    try {
        ...
        std::cout << std::endl << "=== All BLE HID Service Manager tests completed ===" << std::endl;
        return 0;
