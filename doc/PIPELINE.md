...
- **markdown-linting**: Markdown file quality validation
- **mermaid-validation**: Mermaid diagram syntax validation
- **documentation-check**: Doxygen documentation building and validation
- **link-checker**: Documentation link validation
- **quality-summary**: Overall quality gate summary

### 5. Maintenance (`.github/workflows/dependencies.yml`)
...
### CMake Build System
The project uses CMake with make for reliable, parallel builds:
...
# Development build with tests and docs
...
# Memory checking
valgrind --tool=memcheck --leak-check=full ./test_device_manager
```
...
3. ✅ **Code Format**: clang-format compliance
4. ✅ **Markdown**: Markdown linting passes
5. ✅ **Documentation**: Doxygen builds without warnings
6. ✅ **Links**: All documentation links are valid
7. ✅ **Diagrams**: Mermaid diagrams are syntactically correct
8. ✅ **Static Analysis**: No critical issues from clang-tidy/cppcheck
9. ✅ **Security**: No security vulnerabilities detected
10. ✅ **Memory**: No memory leaks detected by Valgrind
...
**Test Failures:**
- Run tests locally: `cd build && ctest --verbose`
- Check for memory leaks: `cd build && valgrind --tool=memcheck ./test_device_manager`
...
The CI pipeline enforces comprehensive quality checks. Run individual checks locally:
```bash
# Code formatting check
clang-format --dry-run --Werror src/*.cpp src/inc/*.hpp

# Markdown linting (requires markdownlint-cli2)
markdownlint-cli2 *.md doc/*.md
```

All quality gates are enforced in CI - no need for local scripts.
...
| `doc/pull_request_template.md` | PR template |
