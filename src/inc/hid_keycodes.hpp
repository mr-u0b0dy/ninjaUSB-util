...
inline const std::unordered_map<int, std::uint16_t> kConsumerUsage = {
    ...
    {KEY_STOPCD, 0x00B7},         {KEY_EJECTCD, 0x00B8},    {KEY_BRIGHTNESSUP, 0x006F},
    {KEY_BRIGHTNESSDOWN, 0x0070}, {KEY_HOMEPAGE, 0x0223},   {KEY_SEARCH, 0x0221},
    {KEY_BACK, 0x0224},           {KEY_FORWARD, 0x0225},    {KEY_REFRESH, 0x0227},
    {KEY_BOOKMARKS, 0x022A},
};
...
class KeyboardState {
  ...
  public:
     ...
     */
    [[nodiscard]] const std::array<std::uint8_t, KEYBOARD_REPORT_SIZE>&
    get_report() const noexcept {
        update_report();
        return report_;
    }
     ...
     */
    [[nodiscard]] std::uint8_t get_modifiers() const noexcept { return modifiers_; }
     ...
     */
    [[nodiscard]] bool is_dirty() const noexcept { return report_dirty_; }
 ...
 */
[[nodiscard]] inline bool apply_key_event(KeyboardState& state, int linux_code,
                                          int value) noexcept {
...
make_consumer_report(int linux_code, int value) noexcept {
    ...
    return {static_cast<std::uint8_t>(usage & 0xFF),
